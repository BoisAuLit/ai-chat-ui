  ...
  async function handleSend() {
    ...
    try {
      const res = await fetch("/api/rag/chat", {
        ...
        body: JSON.stringify({
          ...
          messages: [userMsg, ...messages]
            ...
            .concat([userMsg])
            // dedupe: keep history + latest user
      ...
      while (true) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantMsg.id ? { ...m, text: accumulated } : m,
          ),
        );
      }
    } catch (e) {
  ...
  return (
    <div className="mx-auto flex h-dvh w-full max-w-3xl flex-col p-4">
      <header className="mb-3 flex items-center justify-between border-b border-zinc-200 pb-3 dark:border-zinc-800">
        <div>
          <h1 className="text-lg font-semibold">ai-chat-ui · RAG</h1>
          <p className="text-xs text-zinc-500">
            Paste doc → index → ask. OpenAI embeddings · Claude generation.
          </p>
        </div>
        <Link
          href="/"
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pb-4">
        {!indexed && (
          <p className="text-sm text-zinc-500">Index a document first.</p>
        )}
        {indexed && messages.length === 0 && (
          <p className="text-sm text-zinc-500">
            Ready. Try: <em>&quot;Summarize the key points in 3 bullets&quot;</em> or{" "}
            <em>&quot;What does the doc say about X?&quot;</em>
          </p>
        )}
        ...
        {retrieved.length > 0 && !streaming && (
          <details className="mt-2 rounded-xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900/40">
            <summary className="cursor-pointer select-none px-3 py-2 text-xs font-medium text-zinc-600 dark:text-zinc-400">
              🔍 Retrieved {retrieved.length} chunks for the last question
            </summary>
            <div className="space-y-2 border-t border-zinc-200 px-3 py-3 dark:border-zinc-800">
              {retrieved.map((r) => (
                <div
                  key={r.index}
                  className="rounded-lg border border-zinc-200 bg-white p-2 text-xs dark:border-zinc-800 dark:bg-zinc-950"
                >
